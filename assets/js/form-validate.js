	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#Mangeaille-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#Mangeaille-contact').find('input,textarea').attr({value:''});
				jQuery('#Mangeaille-contact').css({opacity:1});
				jQuery('#Mangeaille-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#Mangeaille-contact').animate({opacity:0.3});
		jQuery('#Mangeaille-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#Mangeaille-contact').find('input,textarea').css('border','none');
